...
def parse_and_modify_feed_items(item, feedurl='/'):
...
def populate_feed(item, cache, feedurl='/', fast=False):
...
def clean_url(url, cache_path, options):
    """ url clean up """
    log(url)
...
def gather(rss, url, cache, options):
    ...
    def worker(i, _item):
        ...
        _item = parse_and_modify_feed_items(_item, url)

        if time.time() - start_time > max_time >= 0 or i + 1 > max_item >= 0:
            if not options.proxy:
                if populate_feed(_item, cache, url, True) is False:
                    _item.remove()
                    return
        else:
            if not options.proxy:
                populate_feed(_item, cache, url)
...
def after(rss, options):
    ...
    if options.json:
        ...
        return rss.to_string(xml_declaration=True, encoding='UTF-8')
...
def process(url, cache=None, options=None):
    ...
    url, cache = clean_url(url, cache, options)
...
def cgi_app(environ, start_response):
    ...
    url, cache = clean_url(url, os.getcwd() + '/cache', options)
...
def cli_app():
    ...
    url, cache = clean_url(url, os.path.expanduser('~/.cache/morss'), options)
    rss = fetch(url, cache, options)
    rss = gather(rss, url, cache, options)
